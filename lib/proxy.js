...
const normalizeUrl = url => _normalizeUrl(url, { stripHash: true, stripWWW: false, stripAuthentication: false });
...
Proxy.prototype.listen = function(options, callback = e => {}) {
  ...
  this.cache = options.cache && {
    ...
    filter: options.cache.filter || (() => true),
    redisClient: null
  };
  ...
  if (hostPort === null) {
    ...
    return self._onRequest(ctx, function(err) {
      ...
      return self._onRequestHeaders(ctx, function(err) {
        ...
        if(self.cache) {
          const key = Proxy.createCacheKey(ctx);
...
var ProxyFinalResponseFilter = function(proxy, ctx) {
  ...
  const shouldCache =
      proxy.cache &&
      !ctx.serverToProxyResponse.fromCache &&
      proxy.cache.filter(ctx.proxyToServerRequest, ctx.serverToProxyResponse);
  ...
  this.end = function(chunk) {
    const onResponseEndCallback = function (err) {
      ...
      if (shouldCache) {
          ...
          const url = Proxy.createCacheKey(ctx);
          const key = url;
...
Proxy.buildUrlFromContext = function (ctx) {
...
Proxy.createCacheKey = function (ctx) {
  return normalizeUrl(Proxy.buildUrlFromContext(ctx));
};
