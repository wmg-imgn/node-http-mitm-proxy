...
Proxy.prototype.listen = function(options, callback = e => {}) {
  ...
  this.cacheRequests = !!options.cacheRequests;
  this.cacheURL = options.cacheURL;
  this.cacheOpts = options.cacheOpts || {};
  if (this.cacheRequests) {
    this.httpCacheableRequest = new CacheableRequest(http.request, self.cacheURL);
    this.httpsCacheableRequest = new CacheableRequest(https.request, self.cacheURL);
  }
...
Proxy.prototype._onHttpServerRequest = function(isSSL, clientToProxyRequest, proxyToClientResponse) {
  ...
  function makeProxyToServerRequest() {
    ...
    function onRequest(request) {
      ctx.proxyToServerRequest = request;
    ...
    const cacheableRequest = ctx.isSSL ? self.httpsCacheableRequest : self.httpCacheableRequest;
    const requestFunction = self.cacheRequests ? cacheableRequest : proto.request;
    const requestOpts = { ...self.cacheOpts, ...ctx.proxyToServerRequestOptions };
    const cacheableOrNormalResult = requestFunction(requestOpts, proxyToServerRequestComplete);
    if (self.cacheRequests) {
      cacheableOrNormalResult.on('request', onRequest);
      cacheableOrNormalResult.on('error', self._onError.bind(self, 'CACHE_REQUEST_ERROR', ctx));
    } else onRequest(cacheableOrNormalResult);
  }
