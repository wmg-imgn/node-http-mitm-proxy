...
const DefaultCacheMaxTTL = 60 * 60 * 3;
...
Proxy.prototype.listen = function(options, callback = e => {}) {
  ...
  if (options.cache) {
    this.cache = {
      redisUrl: options.cache.redisUrl || 'redis://localhost:6379/0',
      maxTtl: options.cache.maxTtl || DefaultCacheMaxTTL,
      redisKeyPrefix: options.cache.redisKeyPrefix || 'proxy-cache',
      cacheFn: options.cache.cacheFn || ((req, res) => true),
      redisClient: null
    };
    const keyFn = options.cache.keyFn || Proxy.createCacheKey;
    this.cache.keyFn = req => `${this.cache.redisKeyPrefix}:${keyFn(req)}`;
  }
  this.forceSNI = !!options.forceSNI;
  this.tlsGlobalKey = options.tlsGlobalKey;
  this.tlsGlobalCert = options.tlsGlobalCert;
  if (this.forceSNI) {
  ...
  ca.create(this.sslCaDir, function(err, ca) {
    ...
    if (self.forceSNI) {
      ...
      self._createHttpsServer({}, function(port, httpsServer, wssServer) {
        ...
        if(self.tlsGlobalKey && self.tlsGlobalCert) {
          self.httpsServer.setSecureContext({key: self.tlsGlobalKey, cert: self.tlsGlobalCert})
        }
      });
    } else {
      self.httpServer.listen(listenOptions, () => {
  ...
  if (this.cache) {
    this.cache.redisClient = redis.createClient({
      socket: {
          url: this.cache.redisUrl
      }
    });
  }
  return this;
  ...
  */
  if (head[0] == 0x16 || head[0] == 0x80 || head[0] == 0x00) {
    ...
    sem.take(function() {
      ...
      if(self.forceSNI && self.tlsGlobalKey && self.tlsGlobalCert){
        release();
        return makeConnection(self.httpsPort);
      }
  ...
  if (hostPort === null) {
    ...
    return self._onRequest(ctx, function(err) {
      ...
      return self._onRequestHeaders(ctx, function(err) {
        ...
        if(self.cache) {
          const key = self.cache.keyFn(ctx.clientToProxyRequest);
          self.cache.redisClient.get(key, (err, result) => {
            if(err || !result) {
              return makeProxyToServerRequest();
            }
            const cachedResponse = JSON.parse(result);
            const responseLike = new ResponseLike(
                cachedResponse.statusCode,
                cachedResponse.headers,
                Buffer.from(cachedResponse.body),
                cachedResponse.url
            );
            responseLike.fromCache = true;
            return proxyToServerRequestComplete(responseLike);
          });
        }
        else return makeProxyToServerRequest();
      });
    });
  }
    ...
    return self._onResponse(ctx, function(err) {
      ...
      return self._onResponseHeaders(ctx, function (err) {
        ...
        try {
          ...
          return self._onError('RESPONSE_WRITEHEAD_ERROR', ctx, err);
        }
...
var ProxyFinalResponseFilter = function(proxy, ctx) {
  ...
  let shouldCache = proxy.cache && !ctx.serverToProxyResponse.fromCache;
  const cacheTime = shouldCache && proxy.cache.cacheFn(ctx.proxyToServerRequest, ctx.serverToProxyResponse);
  shouldCache = shouldCache && !!cacheTime;
  if (shouldCache) this.chunks = [];
  const self = this;
  ...
  this.write = function(chunk) {
    proxy._onResponseData(ctx, chunk, function(err, chunk) {
      ...
      if (chunk) {
        if (shouldCache) self.chunks.push(chunk);
  ...
  this.end = function(chunk) {
    const onResponseEndCallback = function (err) {
      if (err) {
        return proxy._onError('ON_RESPONSE_END_ERROR', ctx, err);
      }
      if (shouldCache) {
          if (chunk) self.chunks.push(chunk);
          const url = proxy.cache.keyFn(ctx.clientToProxyRequest);
          const key = url;
          const buffer = Buffer.concat(self.chunks);
          ...
          proxy.cache.redisClient.set(key, cacheEntry, 'EX', Math.min(cacheTime, proxy.cache.maxTtl)); // errors are ignored on cache saving
      }
      const res = ctx.proxyToClientResponse.end(chunk || undefined);
      return res;
    };
    ...
    if (chunk) {
      return proxy._onResponseData(ctx, chunk, function(err, chunk) {
        ...
        if (shouldCache) self.chunks.push(chunk);
        return proxy._onResponseEnd(ctx, onResponseEndCallback);
      });
    } else {
      return proxy._onResponseEnd(ctx, onResponseEndCallback);
    }
...
Proxy.buildUrlFromRequest = function (req) {
...
Proxy.normalizeUrl = url => normalizeUrl(url, { stripHash: true, stripWWW: false, stripAuthentication: false });

Proxy.createCacheKey = function (req) {
  return normalizeUrl(Proxy.buildUrlFromRequest(req));
};
